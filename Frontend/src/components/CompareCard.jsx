...
const CompareCard = ({ data, title, type = "state" }) => {
  ...
  const riskValue = typeof data?.risk_index !== 'undefined' ? data.risk_index : (data?.risk ?? 'N/A');
  const growthValue = typeof data?.tourism_growth !== 'undefined' ? data.tourism_growth : (data?.growth ?? 'N/A');

  const riskInfo = getRiskLevel(Number(riskValue) || 0);
  const growthInfo = getGrowthTrend(Number(growthValue) || 0);
          ...
          <Col xs={6}>
            <div className="text-center">
              <div className="fs-4 fw-bold text-primary">
                {growthValue !== undefined && growthValue !== null && growthValue !== 'N/A' ? `${growthValue}%` : 'N/A'}
              </div>
          ...
          <Col xs={6}>
            <div className="text-center">
              <div className="fs-4 fw-bold text-warning">
                {riskValue !== undefined && riskValue !== null && riskValue !== 'N/A' ? `${riskValue}/10` : 'N/A'}
              </div>
        ...
        {data?.population && (
          <div className="mt-2">
            <div className="row g-2 text-center">
              {data.population && (
                <div className="col-6">
                  <div className="fw-bold text-primary">{data.population.toLocaleString()}</div>
                  <small className="text-muted">Population</small>
                </div>
              )}
              {data.literacy_rate && (
                <div className="col-6">
                  <div className="fw-bold text-success">{data.literacy_rate}%</div>
          ...
          <div className="mt-3">
            ...
            <div className="d-flex flex-wrap gap-1">
              {data.attractions.slice(0, 3).map((attraction, index) => (
                <Badge key={index} bg="light" text="dark" className="small">{attraction}</Badge>
              ))}
